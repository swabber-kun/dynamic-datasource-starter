...
public class DataSourceTerminationTask implements Runnable {
    ...
    public DataSourceTerminationTask(HikariDataSource dataSource, ScheduledExecutorService scheduledExecutorService) {
    ...
    @Override
    public void run() {
        if (terminate(dataSource)) {
            logger.info("Data source {} terminated successfully!", dataSource);
        } else {
    ...
    private boolean terminate(DataSource dataSource) {
        ...
        try {
            ...
            logger.warn("Terminating data source {} failed, will retry in {} ms, error message: {}", dataSource, RETRY_DELAY_IN_MILLISECONDS, ex.getMessage());
            return false;
        } finally {
            retryTimes++;
        }
    ...
    /**
     * @see <a href="https://github.com/brettwooldridge/HikariCP/issues/742">graceful shutdown of connection pool</a>
     */
    public boolean terminateHikariDataSource(HikariDataSource dataSource) {
